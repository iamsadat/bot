...
* **Real-browser autofill** (`jobhunt/autofill/driver.py`) — a `PlaywrightPage` adapter drives the existing Workday/iCIMS/generic autofillers against a **real browser**, covering the career-page long tail that has no apply API. **Co-pilot by default** (fills the form, leaves the final submit to you); `submit=true` clicks submit only when nothing still `requires_user`. Off unless `JOBHUNT_AUTOFILL_ENABLED=1` (needs Playwright). New `POST /api/autofill`; the autofiller logic stays offline-tested via `FakePage` and a Playwright-guarded end-to-end test verifies the real adapter.
* **Salary & company intel** (`jobhunt/integrations/market.py`) — **salary distribution** (p10/median/p90) for a role+location via Adzuna's histogram (reuses the `ADZUNA_*` keys), surfaced on the résumé-preview drawer; plus **company news intel** (NewsAPI, optional key) scored with the existing news-sentiment heuristic. New `GET /api/salary`, `GET /api/company/intel`, `GET /api/market/status`; offline-tested.
* **Recruiter outreach** (`jobhunt/integrations/enrichment.py`) — find likely recruiter/hiring-manager contacts at a target company (Hunter.io domain-search; Apollo/People-Data-Labs pluggable) via `POST /api/outreach/find`, and draft an **evidence-bound** outreach email via `POST /api/outreach/draft` (references your real matched skills, notes the CV is attached, never fabricates; optional Gemini polish). Send it with the Gmail integration. New `GET /api/outreach/status`; offline-tested.
* **Gmail + Google Calendar** (`jobhunt/integrations/`) — with Google OAuth set, the recruiter-email auto-status uses the **Gmail API** (preferred over IMAP), you can **send follow-up/thank-you emails** (templated) via `POST /api/email/send`, and create **interview holds** on Google Calendar via `POST /api/calendar/hold`. New `GmailSender` + env factories; all offline-tested through the injectable `FakeTransport`. New `GET /api/google/status`.
* **MCP server** (`jobhunt/mcp_server.py`) — expose the pipeline as agent-callable tools so Claude (or any MCP client) can drive a JobHunt instance: `jobhunt_status`, `jobhunt_pipeline`, `jobhunt_list_jobs`, `jobhunt_get_resume`, `jobhunt_search_jobs`, `jobhunt_approve`. Run `python -m jobhunt mcp` (needs `pip install jobhunt[mcp]`); the tool logic is fully offline-tested, the SDK wiring is optional.
* **Notifications & webhooks** (`jobhunt/notify.py`) — fan out high-value events (new matches, auto-applied, interview/assessment from email) to **Slack / Discord / Telegram / generic outbound webhooks** (Zapier/n8n/Make — posts the full structured event) and optional **SMTP email**. Env-configured (any subset), best-effort delivery, every sink offline-tested via `FakePoster`. New `GET /api/notify/status` + `POST /api/notify/test`.
